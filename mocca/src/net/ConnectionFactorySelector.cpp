...
std::vector<ConnectionFactorySelector::ProtocolMessageConnectionFactoryPair> ConnectionFactorySelector::messageConnectionFactories_;
std::vector<ConnectionFactorySelector::ProtocolStreamConnectionFactoryPair> ConnectionFactorySelector::streamConnectionFactories_;
...
void ConnectionFactorySelector::addMessageConnectionFactory(std::unique_ptr<IMessageConnectionFactory> factory,
    ...
    for (auto& fac : messageConnectionFactories_) {
        if (fac.protocol == protocol) {
            throw Error(mocca::formatString("Factory for protocol '%%' already provided to selector", fac.protocol), __FILE__, __LINE__);
        }
    }
...
void ConnectionFactorySelector::addDefaultFactories() {
    // tcp
    addStreamConnectionFactory(std::make_unique<TCPConnectionFactory>(), tcp());

    // queue
    addStreamConnectionFactory(std::make_unique<QueueConnectionFactory>(), queue());

    // tcp.prefixed
    addMessageConnectionFactory(
        std::make_unique<FramingConnectionFactory>(std::unique_ptr<IStreamConnectionFactory>(new TCPConnectionFactory()),
                                                   std::unique_ptr<FramingStrategy>(new SizePrefixedProtocol())),
        tcpPrefixed());

    // queue.prefixed
    addMessageConnectionFactory(
        std::make_unique<FramingConnectionFactory>(std::unique_ptr<IStreamConnectionFactory>(new QueueConnectionFactory()),
                                                   std::unique_ptr<FramingStrategy>(new SizePrefixedProtocol())),
        queuePrefixed());
    ...
    // loopback
    addMessageConnectionFactory(std::make_unique<LoopbackConnectionFactory>(), loopback());
}

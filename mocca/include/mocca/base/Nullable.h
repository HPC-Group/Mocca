...
public:
    ...
    // copy and move
    Nullable(const Nullable& other) : value_(other.value_), isNull_(other.isNull_) {}
    Nullable(Nullable&& other) : value_(std::move(other.value_)), isNull_(other.isNull_) {
        other.isNull_ = true;
    }
    ...
    Nullable(const T& value) : value_(value), isNull_(false) {}
    Nullable(T&& value) : value_(std::move(value)), isNull_(false) {}
    ...
    friend bool operator==(const Nullable<T>& lhs, const T& rhs) {
        return !lhs.isNull_ && lhs.value_ == rhs;
    }
    friend bool operator==(const T& lhs, const Nullable<T>& rhs) {
        return !rhs.isNull_ && lhs == rhs.value_;
    }

...
namespace IVDA
{
	...
	public:
		...
		SocketException(const std::string & message, const std::string & operation, int errorCode)
            : NetworkError(message, __FILE__, __LINE__), m_strOperation(operation), m_iErrorCode(errorCode) {}
		~SocketException() throw() {}

        std::string internalError() const override {
            return m_strOperation + " (error code: " + std::to_string(m_iErrorCode) + ")";
        }
	...
	public:
		...
		uint32_t SendInt(int32_t integer, uint32_t timeout = INFINITE_TIMEOUT);
		uint32_t ReceiveInt(int32_t & integer, uint32_t timeout = INFINITE_TIMEOUT);

...
typename Iter findMemberEqual(Iter it, Iter itEnd, const U IterValueType<Iter>::*Mem, const U& val) {
    for (; it != itEnd; ++it) {
        if (*it.*Mem == val) {
            return it;
        }
...
template <typename Iter, typename U>
typename Iter findMemberEqual(Iter it, Iter itEnd, U (IterValueType<Iter>::*MemFn)() const, const U& val) {
    for (; it != itEnd; ++it) {
        if ((*it.*MemFn)() == val) {
            return it;
        }

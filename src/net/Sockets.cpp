...
namespace IVDA
{
  ...
  uint32_t IOSocket::ReceiveDataImplementation(int8_t * data, uint32_t size, sockaddr * source, socklen_t * sourceSize, uint32_t & timeout)
  {
	...
	uint32_t IOSocket::SendInt(int32_t integer, uint32_t timeout)
	{
		int32_t integerNetwork = htonl(integer);
		return SendData((const int8_t*)&integerNetwork, sizeof(int32_t), timeout); // throws SocketException, SocketConnectionException
	}
	...
	uint32_t IOSocket::ReceiveInt(int32_t & integer, uint32_t timeout)
	{
		int32_t integerNetwork = 0;
		uint32_t bytesReceived = ReceiveData((int8_t*)&integerNetwork, sizeof(int32_t), timeout); // throws SocketException, SocketConnectionException
